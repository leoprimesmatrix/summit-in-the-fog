(function () {
  ...
  function sunDrop(rf)     { return U.smoothstep(18, 68, rf); }     // high in the haze -> below the cloud deck
  function cloudIn(rf)     { return U.smoothstep(28, 44, rf); }     // deck fades in as it descends to meet you
  function duskStars(rf)   { return U.smoothstep(52, 96, rf); }     // pinpricks before the night stack exists
  function moonRise(rf)    { return U.smoothstep(92, 128, rf); }
  function summitNear(rf)  { return U.smoothstep(100, 150, rf); }   // aurora and meteors ramp to the top
  ...
  Sky.drawSun = function (ctx, rf, t, dayA) {
    ...
    var a = dayA * (1 - night) * (1 - U.smoothstep(66, 78, rf));
    if (a <= 0.01) return;
    var k = sunDrop(rf);
    var x = 428 + 36 * k;
    var y = 44 + 262 * k;
  ...
  Sky.drawClouds = function (ctx, rf, t) {
    ...
    if (seaTop < C.H && d > 0) {
      var sunK = sunDrop(rf);
      var sunA = a * wDusk * (1 - U.smoothstep(66, 78, rf)) * U.smoothstep(0.5, 0.9, sunK);
